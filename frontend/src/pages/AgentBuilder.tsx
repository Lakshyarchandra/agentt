...
import {
  Save, Play, Plus, ChevronLeft, Brain, Wrench, MessageSquare, LogIn, LogOut, Trash2,
} from 'lucide-react';
...
const nodeTypes = { llm: LLMNode, tool: ToolNode, prompt: PromptNode, input: InputNode, output: OutputNode };
  ...
  const buildGraphConfig = () => {
    return {
      nodes: nodes.map(({ id, type, position, data }) => {
        const { onChange, ...cleanData } = data as any;
      ...
      <div style={{ flex: 1, display: 'flex' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            ...
            <MiniMap
              nodeColor={(n) => {
                const colors: Record<string, string> = { llm: '#6c63ff', tool: '#10b981', prompt: '#06b6d4', input: '#f59e0b', output: '#a855f7' };
